...
"North America" : df[df['Continent_Name']=="North America"]["Country Name"].unique(),
"South America" : df[df['Continent_Name']=="South America"]["Country Name"].unique(),
...
with item1:
    metric_choro = st.selectbox(
       ...
       'Ease_of_business', 'Exp_Edu_percent_of_GDP', 'Export', 'GDP',
       'Exp_Health_percent_of_GDP', 'Import', 'Industry_percent', 'Inflation',
       'RnD', 'Service_percent', 'Unemployment', 'Population',
        'Exp_Edu_real_value', 'Exp_Health_real_value',
       'Net_trade', 'GDP_Per_Cap'))
...
with choro_map:
    fig5 = px.choropleth(chorodata , locations ="Country Code", color = metric_choro, 
                ...
                color_continuous_scale="Bluered",animation_frame="Year",title = f'{small_exp[metric_choro]} Values In The World',hover_data="Country Name" )
...
with data_df:
    ...
    st.metric("", "","")
    st.metric("", "","")
    st.dataframe(chorodata[chorodata.Year==2022][["Country Name", metric_choro]],height=480)
#### Choropleth MAP Ends ## 
...
with c2:
    calculation = st.selectbox("Calculation Method",("Average", "Total", "Maximum","Minimum")
                               )
...
if calculation=="Average":
    df_to_use1 = df_to_use.groupby(["Continent_Name","Year"])[metric].mean().reset_index()
elif calculation=="Total":
    df_to_use1 = df_to_use.groupby(["Continent_Name","Year"])[metric].sum().reset_index()
elif calculation=="Maximum":
    df_to_use1 = df_to_use.groupby(["Continent_Name","Year"])[metric].max().reset_index()
elif calculation=="Minimum":
    df_to_use1 = df_to_use.groupby(["Continent_Name","Year"])[metric].min().reset_index()
...
with chart1:
    fig = px.line(df_to_use, x = "Year", y = metric, color = "Country Name",
                markers=True, height=800, title = f'{small_exp[metric]} Statistics per Country For The Last 20 Years')
    fig.update_layout(width = 1500,height = 800,)
    st.plotly_chart(fig, theme="streamlit", use_container_width=True)

with chart2:
    fig2 = px.line(df_to_use1, x = "Year", y = metric, markers=True, height=800,color="Continent_Name",title = f'{small_exp[metric]} Statistics per Continent For The Last 20 Years')
...
with chc2:
    ...
    fig4.add_trace(go.Scatter(x = chc_data["Year"], y = chc_data["GDP_Per_Cap"], name = "GDP Per Capita",mode = "markers+lines",
                                line=dict(color = "Cyan")),secondary_y = True)
    fig4.update_layout(
    title_text=f"<b>{small_exp[metric1]}</b> vs <b>GDP Per Capita Comparison</b> In Time", height = 800,legend=dict(x=0.5,y= 1)
    )
    # Set x-axis title
...
with chc1:
    sector_data = df[(df[metric1].isna()==False)&(df["GDP_Per_Cap"].isna()==False)&(df["Year"]==metric3)]

    fig3 = px.scatter(sector_data, x = "GDP_Per_Cap", y = metric1,
                size = "Population", color = "Country Name",height = 800, size_max= 55, title= f"<b>{small_exp[metric1]}</b> vs <b>GDP Per Capita Comparison</b> By Country")
